...
\noindent
This project asks a simple question: can machine-learning and deep-learning models forecast next-day stock-market volatility better than classical statistical models? Eight models were tested on 18.5 years of S\&P~500 daily returns (2008--2026) using a strict walk-forward setup with no look-ahead bias. EGARCH(1,1), a classical model with a leverage term, gave the best result on the main metric (QLIKE), and the leverage models significantly beat plain GARCH(1,1) on a test window containing the 2022 bear market and the April-2025 tariff shock. The LSTM came third, ahead of plain GARCH and statistically tied with the winner, but no learned model beat the leverage GARCH family where it matters. The well-known result that GARCH-type models are very hard to beat on daily data survives, in leverage-aware form.
...
\textbf{Data.} S\&P~500 daily closes (2008--2026, Yahoo Finance), 4{,}650 returns. The last 30\% (1{,}395 days, 2020-12-07 to 2026-06-29, including the 2022 bear market and the April-2025 tariff shock) is the out-of-sample test window shared by every model.

Table~\ref{tab:lb} shows the leaderboard. EGARCH(1,1) wins on QLIKE, statistically tied with GJR-GARCH (Diebold–Mariano $p=0.47$), and both leverage models significantly beat plain GARCH(1,1) ($p<0.0001$). The LSTM ranks third, ahead of plain GARCH, and is statistically tied with the winner ($p=0.19$). The learned models take the point-accuracy metrics (LSTM the best RMSE, XGBoost the best MAE) but rank lower on QLIKE, because their smoothed target makes them accurate on average yet slow to react at volatility spikes. Fig.~\ref{fig:best} shows the winning forecast tracking realized volatility.
...
\midrule
EGARCH(1,1)     & 0.00728 & 0.00556 & \textbf{1.5042} \\
GJR-GARCH(1,1)  & 0.00731 & 0.00554 & 1.5091 \\
LSTM            & \textbf{0.00686} & 0.00499 & 1.5287 \\
GARCH(1,1)      & 0.00751 & 0.00576 & 1.5569 \\
EWMA(0.94)      & 0.00750 & 0.00573 & 1.5814 \\
XGBoost         & 0.00691 & \textbf{0.00496} & 1.5911 \\
...
\section{Discussion}
The leverage models lead because the test window is dominated by two sharp crashes (2022 and April 2025), and their extra term turns a large fall directly into higher next-day variance; on this window that advantage over plain GARCH is highly significant. The GARCH family as a whole stays ahead of the learned models for three reasons: it is fit by maximum likelihood on the right objective, while the learned models train on a smoother substitute target; daily data is small data, which favours simple models over neural networks; and QLIKE rewards fast reactions to spikes, which the GARCH recursion delivers the very next day. The case against the learned models is narrower than on shorter samples, though: the LSTM learned the leverage pattern from data and finished statistically tied with the winner. The main limitations are the single asset and test window, the noisy squared-return proxy, and modest hyperparameter tuning.

\section{Conclusion}
Under a leak-free walk-forward test on 18.5 years of S\&P~500 data, a monthly re-estimated EGARCH(1,1) gave the best one-day-ahead volatility forecasts, the leverage models significantly beat plain GARCH, and no learned model beat the leverage GARCH family on QLIKE. The learned models were competitive (the LSTM statistically tied with the winner) but not better where it counts. The clearest lesson was methodological: a leak-free evaluation and the choice of metric matter more than model complexity. Useful next steps are intraday realized-variance targets, extra inputs such as the VIX, and multi-asset training.

\section{Artifacts}
Code: \url{https://github.com/iitgF/T9} (the \texttt{volforecast} package and notebooks). Video demonstration: \url{https://www.youtube.com/watch?v=SshPU5j-sYM}. LaTeX source (Overleaf): \url{https://www.overleaf.com/project/6a48e18e7dee3049b3ee4209}. Full report: \texttt{report.tex}. AI-assisted tooling was used for refactoring and drafting; all decisions, checks and the final text are the author's, and every number was reproduced from the committed code.
